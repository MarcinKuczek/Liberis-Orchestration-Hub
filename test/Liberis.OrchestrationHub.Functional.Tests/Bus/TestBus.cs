...
namespace Liberis.OrchestrationHub.Functional.Tests.Bus
{
    public sealed class TestBus : IDisposable
    {
        ...
        public async Task StartAsync()
        {
            _busControl = MassTransit.Bus.Factory.CreateUsingRabbitMq(configurator =>
            {
                ...
                configurator.ReceiveEndpoint("Liberis.OrchestrationHub.Functional.Tests-" + Guid.NewGuid(), endpointConfigurator =>
                {
        ...
        {
            // Add publishers configurations manually
        }

        public async Task Publish<T>(T @event, string routingKey) where T : class => await _busControl.Publish(@event, x => x.SetRoutingKey(routingKey)).ConfigureAwait(false);
